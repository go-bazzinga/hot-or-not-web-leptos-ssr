...
use crate::{state::canisters::Canisters, js::wasp::WaspHlsPlayer, component::spinner::Spinner};
...
async fn get_post_uid(canisters: &Canisters, user_canister: Principal, post_id: u64) -> String {
    ...
    // TODO: error handling
    let post_details = post_creator_can.get_individual_post_details_by_id(post_id).await.unwrap();
...
pub fn PostView() -> impl IntoView {
    ...
    let canister_and_post = move || params.with(|p| {
        let go_to_root = || {
    ...
    view! {
            <Suspense fallback=|| view!{
                ...
                {move || video_uid.get()
                    .map(|uid| {
                        let bgurl = format!("https://customer-2p3jflss4r4hmpnz.cloudflarestream.com/{}/thumbnails/thumbnail.jpg", uid);
                        create_effect(move |_| {
                            let url = format!("https://customer-2p3jflss4r4hmpnz.cloudflarestream.com/{}/manifest/video.m3u8", uid);
                            wasp_player.with(|wasp| {
                        ...
                        view! {
                            ...
                            >
                                <div class="grid grid-cols-1 h-screen w-screen justify-items-center backdrop-blur-lg">
                                    <video _ref=video_ref class="object-contain h-screen" loop autoplay muted></video>
                                </div>
                            </div>
                        }
                    })
