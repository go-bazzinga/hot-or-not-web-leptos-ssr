...
#[cfg(feature = "ssr")]
use crate::{state::canisters::Canisters, canister::post_cache};
...
async fn get_top_post_id() -> Result<Option<(Principal, u64)>, ServerFnError> {
    ...
    let top_items = match post_cache.get_top_posts_aggregated_from_canisters_on_this_network_for_hot_or_not_feed(0, 1)
        .await? {
            post_cache::Result_::Ok(items) => items,
            post_cache::Result_::Err(_) => {
                return Err(ServerFnError::ServerError(format!("failed to fetch top post")));
            }
        }; 
        ...
        <Suspense
            ...
            {move || {
                let url = match target_url.get() {
                    Some(Ok(Some((canister, post_id)))) => format!("/hot-or-not/{canister}/{post_id}"),
                    Some(Ok(None)) => format!("/error?err=No Posts Found"),
